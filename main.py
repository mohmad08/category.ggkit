...
function start(c){category=c;mode='learn';list=shuffle(bank[c]);index=0;answers={};revealed={};document.querySelector('#home').classList.remove('active');document.querySelector('#practice').classList.add('active');render()}
function home(){document.querySelector('#practice').classList.remove('active');document.querySelector('#home').classList.add('active')}
function setMode(m){mode=m;index=0;answers={};revealed={};list=mode==='test'?shuffle(bank[category]).slice(0,Math.min(80,bank[category].length)):shuffle(bank[category]);render()}
...
if __name__ == "__main__":
    ...
    print("asdОтeкройте http://127.0.0.1:8000")
